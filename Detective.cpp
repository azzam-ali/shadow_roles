...
void Detective::performAction(vector<Player*>& allPlayers) // this is aggregation because the whole list of players is accessed by detective without owning it
{
    clearScreen();
    ...
    if (cmd == "task")
    {
        ...
        if (gTaskPool.allDone())
            ...
            gTaskPool.doNextTask(); //this is association bec detective is interacting with taskpool without a part whole relationship
    }
    else if (cmd == "inspect")
    {
        string targetName;
