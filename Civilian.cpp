...
void Civilian::performAction(vector<Player*>& allPlayers)//this is aggregation because the whole list of players is accessed by civilian without owning it
{
    (void)allPlayers;//to silence the warning that the allPlayers is passed but not used
    ...
    if (cmd == "task")
    {
        gTaskPool.doNextTask();// this is association bec it is interactingg with taskpool without a part whole relationship
    }
    else if (cmd == "skip")
    {
